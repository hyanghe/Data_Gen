...
def main():
    ...
    directory = './SolverGeneratedData_v03'

    save_data_path = './DeepONetData_LocalTTSS/One_tile_3D_data/'
    ...
    for folder in file_directories:
        ...
        txt_name = "commands_TTSS_tile_heating_4c.txt"
        source_file_name = source_dir + txt_name
        f = open(source_file_name, 'r')
        lines = f.readlines()

        PowerTotal = int(re.findall("\d+", lines[68].split()[0])[0])
        ...
        top_HTC = float(re.findall(match_number, lines[60].split()[-1])[0])
        bottom_HTC = float(re.findall(match_number, lines[61].split()[-1])[0])
        ...
        for TTSS_status_line in TTSS_status_lines:
            if TTSS_status_line.split() and TTSS_status_line.split()[0] == 'TTSS':
                TTSS_status_value = float(re.findall(match_number, TTSS_status_line.split()[1])[0])
                print('TTSS_status_value is: ', TTSS_status_value)
        # TTSS_status_value = float(re.findall(match_number, TTSS_status_lines[62].split()[1])[0])
        if TTSS_status_value < 2000 and TTSS_status_value > 0.1:
            ...
            for line in TTSS_f:
                if isfloat(line.split()[0]) and float(line.split()[0]) < 1e-30:
                    break
