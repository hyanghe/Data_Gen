...
txt_name = "\commands_TTSS_tile_heating_4d.txt"
...
# Total_Power = np.random.randint(low = 1, high=5000, size=(num_cases, 1), dtype=int) * 10**-2
...
Total_Power = np.random.randint(low = 1, high=1501, size=(num_cases, 1), dtype=int) * 10**-2
...
t_Diel = np.random.uniform(low=1.0, high=10.01, size=(num_cases, 1))
t_Insulator = np.random.uniform(low=1.0, high=5.01, size=(num_cases, 1)) * 10**-2
K_Diel = np.random.uniform(low=1.38, high=138.01, size=(num_cases, 1)) * 10**-3
tile_size = np.random.randint(low = 1, high=11, size=(num_cases, 1), dtype=int)
...
params = np.hstack((Total_Power, HTC, Die_xy, Die_z, t_Diel, t_Insulator, K_Diel, tile_size))
...
def create_input_files(params):
    for param in params:
        ...
        PowerTotal = float(format(param[0], '.2f'))
        ...
        for C_scale in C_scale_lst:
            ...
            target_folder_name = dest_fpath + f"Q_{PowerTotal}_HTC_{HTC}_xy_{Die_xy}_z_{Die_z}_tDiel_{t_Diel}_tIns_{t_Insulator}_Kd_{K_Diel}_C_{C_scale}_Tile_{tile_size}\\"
            ...
            lines[15+shift] = lines[15+shift][:10] + f'{tile_size}\n'  ## tile size
            lines[33+shift] = lines[33+shift][:8] + f'{K_Diel}\n' ## Dielectric layer K
            lines[60+shift] = lines[60+shift][:10] + f'{HTC}\n' ## top HTC
            lines[61+shift] = lines[61+shift][:10] + f'{HTC}\n'## bottom HTC
            lines[68+shift] = lines[68+shift][:11] + f'{PowerTotal}\n' ## power

            lines[43+shift] = lines[43+shift][:8] + f'{C_scale}' + lines[43+shift][9:]  ## C_scale
